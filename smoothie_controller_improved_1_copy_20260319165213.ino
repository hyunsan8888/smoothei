...
void handleSerialCommand(String cmd) {
  ...
  if (cmd == "STATUS") {
    ...
    PC_SERIAL.println(F("{\"response\":\"OK\",\"msg\":\"Starting cleaning\"}"));
    DEBUG_SERIAL.println(F("PC 명령: CLEAN 수신 - 세척 시작"));
    safeInitOutputs();
    // ★ 독립 세척 시 값이 0이면 하강 동작 안 함 → 명시적 설정
    if (cleaningStepperPosition == 0) {
        cleaningStepperPosition = STEPS_PER_REV * 14L;  // 14회전
        DEBUG_SERIAL.println(F("  → 스테퍼 기본값: 14회전"));
    }
    if (cleaningDcDownTime == 0) {
        cleaningDcDownTime = 6000;  // DC 6초 하강
        DEBUG_SERIAL.println(F("  → DC 기본값: 6000ms"));
    }
    stepIndex = 0;
...
void loop(){
  ...
  if (runState == CLEANING_PROCESS) {
    switch (stepIndex) {
      ...
      case 2:
        if (millis() - stepTimer >= 7500) {
          DEBUG_SERIAL.println(F("[세척] 7.5초 경과 - 스테퍼 전체 상승 시작"));
          DEBUG_SERIAL.println(F("[세척] 밸브2 계속 ON 상태 유지"));
          // 밸브1 사용 안 함, 밸브2는 계속 ON
          DEBUG_SERIAL.println(F("[세척] 스테퍼 홈까지 상승 시작"));
          // 전체 상승
  ...
  if (runState == RUNNING) {
    ...
    switch (stepIndex) {
      ...
      /* ================= 여기서부터 수정된 부분 (1/3 하강 후 블렌더 시작) ================= */
      
      case 3:
        DEBUG_SERIAL.println(F("[단계 3] ★★★ 스테퍼 3/5 하강 시작 (블렌더 OFF) ★★★"));
        DEBUG_SERIAL.println(F("[단계 3] 총 14회전 중 8회전 먼저 하강"));
        cleaningStepperPosition = STEPS_PER_REV * 14L;
        // 3/5 = 8.4회전이지만 8회전으로 설정 (14 × 3/5 = 8.4)
        stepperStartMove(STEPS_PER_REV * 8L, true);
        stepTimer = millis();
        stepIndex = 30;
        saveStepIndex(30);
        break;
      ...
      case 30:
        ...
        stepperMoveDone();
        if (stepperCurrentStep >= stepperTargetSteps) {
          DEBUG_SERIAL.println(F("[단계 3-0] ★ 3/5 하강 완료 (8회전) ★"));
          DEBUG_SERIAL.println(F("[단계 3-0] ★ 이제 블렌더 시작 ★"));
          delay(500);
          stepIndex = 301;
          saveStepIndex(301);
        }
      ...
      case 301:
        DEBUG_SERIAL.println(F("[단계 3-0-1] ★ 블렌더 예비 회전 시작 (3초) ★"));
        ssrOn(SSR_BLENDER);
        stepTimer = millis();
        stepIndex = 302;
        saveStepIndex(302);
      ...
      case 302:
        ...
        if (millis() - stepTimer >= 3000) {
          DEBUG_SERIAL.println(F("[단계 3-0-2] ★ 블렌더 준비 완료 ★"));
          DEBUG_SERIAL.println(F("[단계 3-0-2] 나머지 10회전 하강 + 블렌딩 시작"));
          stepIndex = 303;
          saveStepIndex(303);
        }
        break;
      ...
      case 303:
        DEBUG_SERIAL.println(F("[단계 3-0-3] ★ 첫 1회전 느리게 하강 시작 ★"));
        DEBUG_SERIAL.println(F("[단계 3-0-3] (재료가 서서히 블렌더로 들어감)"));
      ...
      case 304:
        // 매우 느린 속도로 이동 (첫 1회전 = 약 6초)
        {
          ...
          if (stepperCurrentStep >= stepperTargetSteps) {
            lastBatchTime = 0;
            DEBUG_SERIAL.println(F("[단계 3-0-4] ★ 느린 1회전 완료 (6초 소요) ★"));
            DEBUG_SERIAL.println(F("[단계 3-0-4] ★ 이제 정상 속도로 전환 ★"));
            delay(500);
      ...
      case 40:
        ...
        if (millis() - stepTimer >= 1000) {
          ...
          ssrOff(SSR_BLENDER);
          delay(200);
          DEBUG_SERIAL.println(F("[단계 3-11] 블렌더 다시 ON (나머지 4회전 하강)"));
          ssrOn(SSR_BLENDER);
          delay(200);
          DEBUG_SERIAL.println(F("[단계 3-12] 스테퍼 나머지 하강 시작 (4회전 = 18초)"));
          stepperStartMove(STEPS_PER_REV * 4L, true);  // 4회전 (12,800 스텝)
